...
public:
    ...
    void execute_command_servo(Command command) {
        std_msgs::msg::String msg;
        msg.data = "servoj(get_inverse_kin(p[" + std::to_string(command.position.x) +"," +std::to_string(command.position.y)+"," +std::to_string(command.position.z)+"," +std::to_string(command.orientation.x)+"," +std::to_string(command.orientation.y)+","+ std::to_string(command.orientation.z) +  "]), 0, 0, 4, 0.1, 300)"; 
        script_command_pub_->publish(msg);
...
{
public:
    ActionManager() : Node("ActionManager"),ur3_controller_(new UR3Controller(this)) ,command_key(" "), offset(0.047), object_in_gripper(false), picked_(false), placed_(false), approach(false)
    {
        setup_subscribers();
        initialize_commands();
...
private:
    ...
    void setup_subscribers() {
        ...
        robot_position_sub_ = this->create_subscription<geometry_msgs::msg::Point>("robot_position", 1, std::bind(&ActionManager::handPositionCallback, this, std::placeholders::_1));
        ...
        hand_rate_of_change_sub_ = this->create_subscription<geometry_msgs::msg::Point>("hand_rate_of_change", 1, std::bind(&ActionManager::handRateOfChangeCallback, this, std::placeholders::_1));
